...
#----- physical constants---------
seed = int(time.time() * 1000) % 10000  # Unique ~0–9999
 ...
 #     W_sys_peak = SEC_sys_at_peak * vap_h2_pdt_peak * MH2 * 3600 # peak allowable input power to system, 10% above the rated curr dens.
    ...
    # show x vector in the figure
     ...
     # STACK
      ...
      pv_powerout_scaled = pv_powerout * genratio*rel_RES * (W_sys75)
      wind_powerout_scaled = wind_powerout * genratio*(1-rel_RES) * (W_sys75)*(1/1504.5) # to get the correct mean annual energy produced including wind effects
...
#      degrate_eff1 = degrate* (1.0 + alpha * starts1)
      ...
      m_scale =0.9
      PV_basecapex = 900 # $/kW
      PV_capex = (PV_basecapex*(100*1000)*((genratio*(rel_RES) * (W_sys75)*0.001)/(100))**(m_scale))/(genratio*(rel_RES) * (W_sys75)) # scaled capex based on installed renew capacity
      wind_basecapex = 1500 # $/kW
      wind_capex = (wind_basecapex*(100*1000)*((genratio*(1-rel_RES) * (W_sys75)*0.001)/(100))**(m_scale))/(genratio*(1-rel_RES) * (W_sys75)) # scaled capex based on installed renew capacity
      PV_opex = 17
      wind_opex =44
      ...
      CF_wind = np.sum(wind_powerout_scaled)*0.001/((genratio*(1-rel_RES) * (W_sys75))*8760) 
      CF_solar = np.sum(pv_powerout_scaled)*0.001/((genratio*(rel_RES) * (W_sys75))*8760)
           ...
           for ratio_val in cdeta_sweep_vals:
            #print(f"--- GA run for Tmax_constraint = 5.0, cd_eta75 base ratio = {ratio_val} ---")
                fname = (
                  ...
                  f"_rel_RES{rel_RES:.1f}"
                    f"_job{jobid}_SECsys_gc3x.mat"
                )
                ...
                res = minimize(
                        ...
                        seed=seed, verbose=True #, callback=log_callback
                )
      ...
      #W_refrcomp     = y_grid_unscaled[:, self.W_refrcomp_IDX]*gc
                    ...
                    ###########################################################################     AUXILLIARY CAPEX CALCULATION       ###########################################
                       ...
                       #     self.xk += 1
                        ...
                        #f_min = 0.3; #10% load factor
                        min_load = cd_eta75_base_ratioo*cd_eta75
                  ...
                  #      load_factor1 = E_effective1 / theoretical_max*100*plant_availability
                        ...
                        degrate = base_degrate + m_vall*(np.maximum(maxT,1)**njj)*np.exp(bj*x[-1])  # base_degrate is 1% per annum
                        alphaa = sw*2*5*10**-4 
                        degrate_eff = degrate* (1.0 + alphaa * n_starts)  # degrate = 0.125  + m_degrate * max(maxT_interp - T_thresh, 0).^3.*exp(0.00001*xFine); degradation rate is % per annum
