...
outputVars = {'SEC_sys', 'SEC_stack', 'eta_stack', 'eta_volt', ...
...
for k = 1:length(matFiles)
    ...
    for v = 1:length(outputVars)
        ...
        data.Q_lyeheater = heater_lye+heater_anlye;
        %idx_first = find(data.T_gl_out>Tin, 1, 'first'); % to check at
        %index the T out of the glsep get greater then Tin
        Cp_cw = 4184; % J/kgK
        T_cws = 293.15;
        T_cwr = T_cws + 5;
        ...
        m_ref_deoxo_cooler  = (m_cw_deoxo_cooler*Cp_cw*(T_cwr-T_cws)./refr_lat_evap);
        refrig_deoxo= (delH_comp.*m_ref_deoxo_cooler./(eta_mech_comp*eta_mech_motor)+((m_ref_deoxo_cooler.*((cond_duty)))./(delT_air*Cp_air*rho_air)).*delP_fan./eta_fan);
        ...
        W_deoxo = (refrig_deoxo +heater_deoxo +heater_regen);
        ...
        p_vap_reg_eff = P.*(data.x_H2O_h2cool_out+alpha_purge.*(xH2O_reg_out-data.x_H2O_h2cool_out)); %Effective partial pressure of moisture in regen
        b_reg = ((rq.^0.2472)./(1-rq.^0.2472)).^(1/0.2472).*(1./p_vap_reg_eff);
        T_reg = max(T_glsepHXout+75,(1/(293.15)+R_const./H_ads.*log(b_reg/b_ref)).^-1);
        heater_ads = (purge_ads)*Cp_H2_80*(T_reg-T_glsepHXout(ones(length(T_reg),1)))+des_rate*H_ads+(m_steel_ads*Cp_steel+m_ads*Cp_ads).*(((T_reg-T_glsepHXout(ones(length(T_reg),1)))./t_des)/eta_heater)*t_des/t_b;% heater duty in watts, avegared over a complete cycle. ERROR in the heat_ads formula, used in the GA and here... moisture is not considered
        ...
        m_ref_ads_cooler = (m_cw_cond_ads*Cp_cw*(T_cwr-T_cws)./refr_lat_evap); 
        refrig_ads= (delH_comp.*m_ref_ads_cooler./(eta_mech_comp*eta_mech_motor)+((m_ref_ads_cooler.*((cond_duty)))./(delT_air*Cp_air*rho_air)).*delP_fan./eta_fan);
        %W_ads =     (refrig_ads+heater_ads);

        data.SEC_ads = (refrig_ads +heater_ads)*J_kWh./(vap_h2_pdt*MH2);

        data.SEC_refrig_deoxo = refrig_deoxo*J_kWh./(vap_h2_pdt*MH2);

        data.SEC_refrig_ads = refrig_ads*J_kWh./(vap_h2_pdt*MH2);
        
        data.SEC_heater_ads = heater_ads*J_kWh./(vap_h2_pdt*MH2);
        data.SEC_heater_deoxo = heater_deoxo*J_kWh./(vap_h2_pdt*MH2);
        data.SEC_heater_regen = heater_regen*J_kWh./(vap_h2_pdt*MH2);

        m_ref_h2cooler = (m_cw_h2cooler*Cp_cw*(T_cwr-T_cws)./refr_lat_evap); 

        refrig_h2cooler= (delH_comp.*m_ref_h2cooler./(eta_mech_comp*eta_mech_motor)+((m_ref_h2cooler.*((cond_duty)))./(delT_air*Cp_air*rho_air)).*delP_fan./eta_fan);
        ...
        data.W_pumps = W_pump_dmw + W_pump_oxlye +W_pump_hylye +  W_pump_cws;
       ...
       % data.W_refrcomp+data.W_refrfan
        ...
        data.W_chiller = W_refrcomp+W_refrfan;
        W_H2purif = (data.SEC_ads + data.SEC_deoxo).*vap_h2_pdt*3600*0.002*1000;
        ...
        data.W_compr = W_comp + W_fan;
        W_sys=((data.W_stack + W_H2purif + W_comp + W_fan + W_pump_cws + W_pump_dmw + W_pump_oxlye + W_pump_hylye + heater_lye + heater_anlye + W_refrcomp+W_refrfan));
        
        
        SEC_sys = ((W_sys))*J_kWh./(vap_h2_pdt*MH2);
        SEC_compr = (W_comp+W_fan)*J_kWh./(vap_h2_pdt*MH2);  
        data.SEC_compr = SEC_compr;
        data.SEC_sys = SEC_sys;
        eta_sys = (HHV_H2 ./ SEC_sys) * 100;
        data.eta_sys = eta_sys;
        data.W_sys = W_sys;
        data.glsep_O2 = glsep_O2;
        data.SEC_H2purif  = data.SEC_ads + data.SEC_deoxo;
        data.W_H2purif = data.SEC_H2purif.*vap_h2_pdt*3600*0.002*1000;
        data.SEC_glsep = data.SEC_h2cooler; 

%%%%%%%%%%% Finding the rated power values for the stack and aux components
%%%%%%%%%%% to populate the Sankey Diagram%%%%%%%%%%%%%%%%%%

SankeyNames = {'W_sys', 'W_stack', 'W_elec_heater', 'W_compr', 'W_pumps', 'W_H2purif',...
    'Q_evapcool','Q_stackgases','Q_ambientcool','W_shuntcurr','Q_glsepcooling' , 'Q_lyecooler','Q_h2_pdt','frac_H2lost','W_chiller','SEC_ads','SEC_refrig_deoxo','SEC_refrig_ads','SEC_heater_ads','SEC_heater_deoxo','SEC_heater_regen','SEC_compr','Q_lyeheater'};

       data.W_elec_heater = heater_anlye + heater_lye + heater_ads +heater_deoxo + heater_regen;
       data.Q_glsepcooling   =data.heat_loss_glseps;
       data.Q_evapcool = data.heat_loss_evapcell     ;
       data.Q_stackgases = data.heat_loss_gasglsep;
       data.Q_ambientcool = data.heat_loss_stack;
       data.W_shuntcurr = (1-data.eta_curr_stack/100).*data.W_stack;
       data.Q_h2_pdt = 285.8*data.vap_h2_pdt*1000; % energy contained in the product hydrogen in Watts
       data.Q_h2_stack = data.W_stack.*data.eta_stack/100;% energy contained in h2 produced in the stack
       data.frac_H2lost = 1-data.Q_h2_pdt./data.Q_h2_stack;
...
for vv = 1:numel(SankeyNames)
    vName = SankeyNames{vv};
    ...
    % You can optionally handle other cases (arrays/matrices) here
        ...
        eta_stack_interp = interp1(data.currentDensity, data.eta_stack, xFine, 'pchip', 'extrap');

       % eta_stack_interp = linspace(min(data.eta_stack), 15000, 500);
...
% SankeyNames = {'W_sys', 'W_stack', 'W_elec_heater', 'W_comp', 'W_pumps', ...
%     'Q_evapcool','Q_stackgases','Q_ambientcool','W_shuntcurr','Q_glsepcooling' , 'Q_lyecooler','Q_h2_pdt','frac_H2lost'};

fprintf('Current density at 70%% stack efficiency (high-current side): %.2f A/m^2\n', cd_eta75);   

for kk = 1:numel(SankeyNames)
    base   = SankeyNames{kk};          % e.g. 'W_sys'
    interp = [base '_interp'];        % 'W_sys_interp'
    out    = [base '75'];             % 'W_sys75'

    % assumes variables like W_sys_interp exist in workspace
    val75 = interp1(xFine, eval(interp), cd_eta75, 'linear', 'extrap');
    assignin('base', out, val75);
end
    ...
    allInputs(:,:) = inputMatrix;
    allOutputs = thisOutputs;
end
...
currDensCol = 6; % current density is in column 6
SankeyNames75 = strcat(SankeyNames,'75');      % {'W_sys75', 'W_stack75', ...}

ratedVals = zeros(1,numel(SankeyNames75));     % preallocate

for k = 1:numel(SankeyNames75)
    ratedVals(k) = eval(SankeyNames75{k});     % assumes W_sys75, W_stack75, ... exist
end

T = array2table(ratedVals,'VariableNames',SankeyNames75);
writetable(T,'rated_Sankey_values.xlsx');
...
%allOutputs(:, nanCols) = [];
%allInputsScaled(isnan(allInputsScaled)) = 0;
% ind_cmplx = (imag(allOutputs(:,22))~=0);
% ele = ind_cmplx.*(allOutputs(:,1));
% ele(ele==0)=[];
% ffd = ind_cmplx.*(allInputs(:,6));
% ffd(ffd==0)=[];

% Save scaled data and scaling parameters for future inverse transform
%save('preparedDatanew_scaled.mat', 'allInputsScaled', 'allOutputsScaled', ...
  %  'minIn', 'maxIn', 'minOut', 'maxOut');
save('preparedData_SEC.mat', 'allInputs', 'allOutputs')
