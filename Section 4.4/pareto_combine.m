...
format long g
 ...
 %files = dir('ga_sens_cdeta0.*_m0.0010_nj0.5_PV_rated75.0_gr3.5_lcoh_job9778642_SECsys.mat');
  files = dir('ga_sens_currdens1100.000_cdeta0.28_m0.0010_nj0.5_PV_rated75.0_gr3.0_lcoh_job10725359_SECsys_.mat');                 % all GA sensitivity files
 %files = dir('ga_sens_sepwidth*_cdeta0.28_m0.0010_nj0.5_PV_rated75.0_gr3.0_lcoh_job*_SECsys_.mat');                 % all GA sensitivity files
...
for i = 1:nF
    ...
    % NEW: parse swept parameter name and value from prefix
    % examples: ga_sens_elecwidth0.7_cdeta..., ga_sens_currdens2000_cdeta...
    t = regexp(name,'^ga_sens_([a-zA-Z]+)([0-9\.]+)_','tokens','once');
    if isempty(t)
        error('Could not parse swept parameter from file name: %s', name);
    end
    params(i).parName = string(t{1});          % 'elecwidth','currdens',...
    params(i).parVal  = str2double(t{2});      % numeric value
end
...
PV_v     = [params.PV]';
gr_v     = [params.gr]';
parName_v= [params.parName]';
parVal_v = [params.parVal]';

Tpar = table(cdeta_v,m_v,nj_v,PV_v,gr_v,parName_v,parVal_v, ...
    'VariableNames',{'cdeta','m','nj','PV','gr','parName','parVal'});
...
for g = 1:nGroups
 ...
 %   all_LCOE_awe= [];
   ...
   % k
outName = sprintf('combined_paramSens_%s%.4g_cdeta%.3f_m%.4f_nj%.2f_PV%.1f_gr%.1f.mat', ...
                  Tu.parName(g), Tu.parVal(g), ...
